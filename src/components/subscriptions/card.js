...
const SubscriptionPaymentDate = ({ subscription, formatDate }) => {
  if (!subscription.enabled) {
    return undefined;
  }

  const paymentDate = toZonedTime(new Date(subscription.paymentDate), subscription.timezone);
  const isPast = DateFNS.isPast(paymentDate);
  return (
...
const SubscriptionIsNotified = ({ subscription, formatDate }) => {
  if (subscription.enabled && subscription.nextNotificationTime) {
    return (
      <div>
        ...
        {' '}
        {formatDate(subscription.nextNotificationTime)}
...
const SubscriptionPaymentCount = ({ subscription }) => {
  ...
  const getPaymentCount = () => {
    const startDate = toZonedTime(new Date(subscription.paymentDate), subscription.timezone);
    const endDate = toZonedTime(new Date(subscription.untilDate), subscription.timezone);
      ...
      <TooltipProvider delayDuration={300}>
        <Tooltip>
          ...
          <TooltipContent>
            Until
            {' '}
            {DateFNS.format(new Date(subscription.untilDate), 'dd MMMM yyyy, HH:mm')}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    </div>
  ...
  const formatDate = (date) => {
    return (
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div className='inline-flex'>{DateFNS.formatDistanceToNowStrict(DateFNS.addDays(new Date(date),0), {addSuffix: true})}</div>
          </TooltipTrigger>
          <TooltipContent>
            {DateFNS.format(new Date(date), 'dd MMMM yyyy, HH:mm')}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
    );
      ...
      <CardContent className='grow'>
        <div className='flex flex-col gap-2 items-start justify-center text-left'>
          {subscription.enabled ? (
            <>
              <SubscriptionPaymentDate subscription={subscription} formatDate={formatDate} />
              <SubscriptionMarkAsPaid subscription={subscription} />
              <SubscriptionPaymentCount subscription={subscription} />
              <SubscriptionIsNotified subscription={subscription} formatDate={formatDate} />
            </>
          ) : (
