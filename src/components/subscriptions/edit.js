...
import { SubscriptionActionEdit } from '@/components/subscriptions/actions';
...
export const SubscriptionEdit = ({ user, subscription = undefined, categories = [] }) =>  {
  const form = useForm({
    ...
    defaultValues: {
      ...
      paymentDate: subscription?.paymentDate ? toZonedTime(subscription.paymentDate, subscription.timezone) : addDays((new Date()).setHours(9, 0, 0, 0), 1),
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='w-full max-w-2xl flex flex-col gap-6 text-left'>
        ...
        <div className='flex justify-left'>
          <Button type='submit'>{subscription ? 'Update Subscription' : 'Create Subscription'}</Button>
        </div>
      </form>
    </Form>
  )
