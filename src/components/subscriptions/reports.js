...
export function SubscriptionReports({ subscriptions }) {
  const stats = useMemo(() => {
    ...
    const calculateUpcomingPayments = (sub) => {
      const payments = [];
      let currentDate = sub.paymentDate;

      while (currentDate) {
        const nextPaymentDate = GetNextPaymentDate(currentDate, sub.untilDate, sub.cycle.time, sub.cycle.every);
        if (!nextPaymentDate || nextPaymentDate > endOfNextYear) break;
    ...
    const costs = activeSubscriptions.reduce((acc, sub) => {
      ...
      const paymentThisMonth = upcomingPayments.filter(payment => payment.date >= now && payment.date <= beginningOfNextMonth);
      const paymentInOneMonth = upcomingPayments.filter(payment => payment.date >= now && payment.date <= nextMonth);
      const paymentNextMonth = upcomingPayments.filter(payment => payment.date >= beginningOfNextMonth && payment.date <= endOfNextMonth);

      const paymentThisYear = upcomingPayments.filter(payment => payment.date >= now && payment.date <= beginningOfNextYear);
      const paymentInOneYear = upcomingPayments.filter(payment => payment.date >= now && payment.date <= nextYear);
      const paymentNextYear = upcomingPayments.filter(payment => payment.date >= beginningOfNextYear && payment.date <= endOfNextYear);

      const thisMonthAmount = paymentThisMonth.reduce((sum, payment) => sum + payment.price, 0);
      const inOneMonthAmount = paymentInOneMonth.reduce((sum, payment) => sum + payment.price, 0);
      const nextMonthAmount = paymentNextMonth.reduce((sum, payment) => sum + payment.price, 0);
      const thisYearAmount = paymentThisYear.reduce((sum, payment) => sum + payment.price, 0);
      const inOneYearAmount = paymentInOneYear.reduce((sum, payment) => sum + payment.price, 0);
      const nextYearAmount = paymentNextYear.reduce((sum, payment) => sum + payment.price, 0);

      if (!acc.thisMonth) acc.thisMonth = { total: {}, categories: {} };
      if (!acc.inOneMonth) acc.inOneMonth = { total: {}, categories: {}, payments: {} };
      if (!acc.nextMonth) acc.nextMonth = { total: {}, categories: {} };
      if (!acc.thisYear) acc.thisYear = { total: {}, categories: {} };
      if (!acc.inOneYear) acc.inOneYear = { total: {}, categories: {} };
      if (!acc.nextYear) acc.nextYear = { total: {}, categories: {} };
      if (!acc.mostExpensive) acc.mostExpensive = {};

      // Track most expensive subscription per currency for monthly and yearly
      if (!acc.mostExpensive.monthly) acc.mostExpensive.monthly = {};
      if (!acc.mostExpensive.yearly) acc.mostExpensive.yearly = {};

      // Add payments for one month grouped by date and currency
      paymentInOneMonth.forEach(payment => {
      ...
      // Handle most expensive monthly
      if (inOneMonthAmount > 0) {
        if (!acc.mostExpensive.monthly[sub.currency]) {
          acc.mostExpensive.monthly[sub.currency] = {
            name: sub.name,
            amount: inOneMonthAmount,
            logo: logo
          };
        } else if (inOneMonthAmount > acc.mostExpensive.monthly[sub.currency].amount) {
          acc.mostExpensive.monthly[sub.currency] = {
            name: sub.name,
            amount: inOneMonthAmount,
            logo: logo
      ...
      // Handle most expensive yearly
      if (inOneYearAmount > 0) {
        if (!acc.mostExpensive.yearly[sub.currency]) {
          acc.mostExpensive.yearly[sub.currency] = {
            name: sub.name,
            amount: inOneYearAmount,
            logo: logo
          };
        } else if (inOneYearAmount > acc.mostExpensive.yearly[sub.currency].amount) {
          acc.mostExpensive.yearly[sub.currency] = {
            name: sub.name,
            amount: inOneYearAmount,
            logo: logo
      ...
      const updateCosts = (period, amount) => {
        if (amount !== 0) {
          ...
          // If subscription has no categories, add to 'Uncategorized'
          const categoriesToUse = sub.categories.length > 0 ? sub.categories : [{ name: 'Uncategorized' }];

          categoriesToUse.forEach(category => {
            if (!acc[period].categories[category.name]) {
              acc[period].categories[category.name] = {};
            }
            acc[period].categories[category.name][sub.currency] = (acc[period].categories[category.name][sub.currency] || 0) + amount;
          });
        }
      };

      updateCosts('thisMonth', thisMonthAmount);
      updateCosts('inOneMonth', inOneMonthAmount);
      updateCosts('nextMonth', nextMonthAmount);
      updateCosts('thisYear', thisYearAmount);
      updateCosts('inOneYear', inOneYearAmount);
      updateCosts('nextYear', nextYearAmount);
